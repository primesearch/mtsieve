...
BigHashTable::BigHashTable(uint32_t elements) : HashTable()
{
   ...
   htable = (uint32_t *) xmalloc(hsize, sizeof(uint32_t), "htable");
   olist = (uint32_t *) xmalloc(elements, sizeof(uint32_t), "olist");
...
void    BigHashTable::DumpTables(uint64_t thePrime)
{
   uint32_t idx;
